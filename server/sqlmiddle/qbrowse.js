...
const getqbrowse = async (ctx, next) => {
  ...
  try {
    const data = await mysql("qbrowse").where('openId', ctx.request.query["openId"]).limit(pageSize).offset(page * pageSize).orderBy('qbrowse.sort', 'desc').leftJoin('QUESTION', 'qbrowse.tId', 'QUESTION.id');
    handleData(ctx, data);
    await next()//执行下一个中间件
  } catch (e) {
    ctx.body = {
